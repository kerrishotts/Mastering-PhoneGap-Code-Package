...
function performSubstitutions() {
  ...
  return replace(
    {patterns: [
      ...
      {match: /{{{PREFS}}}/g, replacement: transformCordovaPrefs},
      {match: /{{{ICONS}}}/g, replacement: transformCordovaAssets.bind(null,"icon",0)},
      {match: /{{{SPLASHES}}}/g, replacement: transformCordovaAssets.bind(null,"splash",0)},
      {match: /{{{GAP:PLUGINS}}}/g, replacement: transformCordovaPlugins},
      {match: /{{{GAP:ICONS}}}/g, replacement: transformCordovaAssets.bind(null,"icon",1)},
      {match: /{{{GAP:SPLASHES}}}/g, replacement: transformCordovaAssets.bind(null,"splash",1)}
    ]}
  );
}
...
var cordovaTasks = {
  ...
  build: function build() {
    ...
    return cordova.build({options: ["--"+BUILD_MODE]})
                  .then(logDone);
  },
  /**
  ...
  emulate: function emulate() {
    ...
    return cordova.emulate({platforms: [PLATFORM], options: ["--"+BUILD_MODE]})
                  .then(logDone);
  },
  /**
  ...
  run: function run() {
    ...
    return cordova.run({platforms: [PLATFORM], options:["--device", "--" + BUILD_MODE]})
                  .then(logDone);
  }
};
...
var projectTasks = {
  ...
  lintCode: function lintCode() {
    return gulp.src(["./src/www/js/app/**/*"])
               .pipe(eslint({ configFile: path.join(CONFIG_DIR, "eslint.json")}))
               .pipe(eslint.format())
...
var tasks = {
  ...
  "init": { deps: ["clean"], task: projectTasks.init },
  "copy-assets": projectTasks.copyAssets,
  "copy-config": projectTasks.copyConfig,
  "copy-code": projectTasks.copyCode,
  "copy": { deps: ["copy-assets", "copy-code"]},
  "build": { deps: ["copy"], task: projectTasks.buildCordova },
  "emulate": { deps: ["copy"], task: projectTasks.emulateCordova },
  "run": { deps: ["copy"], task: projectTasks.runCordova },
