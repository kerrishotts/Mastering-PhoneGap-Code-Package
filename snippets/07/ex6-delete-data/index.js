...
let lemmas = [
  ...
  db.transaction((tx) => {
      ...
      return db.transaction((tx) => {
          tx.executeSql("DELETE FROM lemmas WHERE lemma = ?", ["cat"]);
          tx.executeSql("SELECT definition.wordNetRef, lemma, partOfSpeech, gloss FROM definition, lemmas WHERE definition.wordNetRef = lemmas.wordNetRef AND lemma = ? ORDER BY lemma",
          ["cat"], (tx, results) => {
              let rows = [];
  ...
  db.transaction((tx) => {
    ...
    return db.exec({sql:"DELETE FROM lemmas WHERE lemma = ?", binds: ["cat"]});
  }).then(() => {
      return db.exec({sql:"SELECT definition.wordNetRef, lemma, partOfSpeech, gloss FROM definition, lemmas WHERE definition.wordNetRef = lemmas.wordNetRef AND lemma = ? ORDER BY lemma",
                      binds: ["cat"]});
